...
fun Vibrator.vibrate() {
    if (beforeO) {
        @Suppress("DEPRECATION") vibrate(DURATION_IN_MS)
    } else {
        vibrate(
            VibrationEffect.createOneShot(
                DURATION_IN_MS, VibrationEffect.DEFAULT_AMPLITUDE
            )
        )
    }
}
...
fun Vibrator.error() {
    if (beforeO) {
        @Suppress("DEPRECATION") vibrate(errorPatternTimings, -1)
    } else {
        vibrate(
